...
def Load_data_cv(cv=10, clean_string=True):
    df=pd.read_csv('/home/polo/.config/spyder-py3/personality-detection-master/essays.csv',encoding='iso-8859-1')
    revs = []
    vocab = defaultdict(float)
    #for line in df['TEXT'].tolist():
    for index, line in df.iterrows():
        status=[]
        sentences=re.split(r'[.?]', line['TEXT'].strip())
        try:
            sentences.remove('')
        except ValueError:
               None
        for sent in sentences:
            if clean_string:
               ...
               for word in words:
                   vocab[word] += 1
#               print(sent)
#               datum  = {"y0":1 if line['cEXT'].lower()=='y' else 0,
#                         "y1":1 if line['cNEU'].lower()=='y' else 0,
#                         "y2":1 if line['cAGR'].lower()=='y' else 0,
#                         "y3":1 if line['cCON'].lower()=='y' else 0,
#                         "y4":1 if line['cOPN'].lower()=='y' else 0,
#                         "text": status, "user": line[0],
...
def load_bin_vec(fname, vocab):
    ...
    with open(fname, "rb") as f:
        ...
        for line in xrange(vocab_size):#df,sentences
...
def get_mairesse_features(file_name):
    ...
    with open(file_name, "rb") as csvf:
        csvreader=csv.reader(csvf,delimiter=',',quotechar='"')
        for line in csvreader:
            feats[line[0]]=[float(ftheano) for f in line[1:]]
    return feats
...
#
data_folder = '/home/polo/.config/spyder-py3/personality-detection-master/essays.csv'
revs, vocab = Load_data_cv(cv=10, clean_string=True)
#vocab,df = Load_data_cv(cv=10, clean_string=True)